...
date: "February 2015"
output: pdf_document
title: "Analysis of microarray data for biennial bearing in apple"
---
...
```{r Cluster_analysis, echo=TRUE, fig.height=5, fig.width=5, fig.align='center'}
...
```{r Cluster_dendogram, echo=TRUE, fig.height=10, fig.width=10, fig.align='center'}
# Definition of the number of clusters
nb_groups <- 11

# Plot the tree
classif2 <- as.dendrogram(classif1)
plot(classif2, leaflab = "none", xlab = "Genes", ylab = "Height", main = "Fig.11. Cluster dendogram of transcripts with statistically significant differences in expression. \n Blue line represents where the tree has been cut to get 11 clusters")
...
heatmap_matrix <- as.matrix(cluster_data[,3:5])
rownames(heatmap_matrix) <- cluster_data$genes_seq_id

library(gplots)

heatmap.2(t(heatmap_matrix), scale(cluster_data[,3:5]), dendrogram="col", col=heat.colors(5), density.info="none", trace="none", labCol = list(NULL), labRow = NULL, srtRow = NULL, srtCol = list(NULL), distfun = dist, hclustfun = hclust, reorderfun=function(d, w) reorder(d, w, agglo.FUN = mean))
```
...
```{r Graph_interaction_plot, echo=TRUE, eval=FALSE, fig.width=7, fig.height=9}
...
```{r creation_of_file_per_cluster, fig.width=12, fig.height=14}
